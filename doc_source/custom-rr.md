...
You can create Oracle replicas for RDS Custom for Oracle DB instances\. Both container databases \(CDBs\) and non\-CDBs are supported\. Creating an Oracle replica is similar to the process in Amazon RDS, but with important differences\. For general information about creating and managing Oracle replicas, see [Working with DB instance read replicas](USER_ReadRepl.md) and [Working with read replicas for Amazon RDS for Oracle](oracle-read-replicas.md)\.
